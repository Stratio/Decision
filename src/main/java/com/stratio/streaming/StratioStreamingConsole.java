            ...
            public void eventReceived(CuratorFramework client, CuratorEvent event) throws Exception {
                ...
                case WATCHED:
                    ...
                    try {
                        ...
                        System.out.println("<<<<<<<<<< REPLY FROM STRATIO STREAMING FOR REQUEST ID: "
    ...
    private void printListStreams(String originalReply) {
        ...
        for (StratioStreamingMessage stream : reply.getStreams()) {
            System.out.println("********" + stream.getStreamName()
                    + printColumnsAndQueries(stream.getColumns(), stream.getQueries()));
        }
        System.out.println("*******************************************");
        System.out.println("");
    }
    ...
    private void handleCommand(String request) {
        ...
        try {
            ...
            KeyedMessage<String, String> busMessage = new KeyedMessage<String, String>(BUS.TOPICS,
                    request.split("@")[0].trim(), new Gson().toJson(message));
            producer.send(busMessage);

            client.checkExists().watched()
                    .forPath(STREAMING.ZK_BASE_PATH + "/" + message.getOperation() + "/" + message.getRequest_id());
    ...
    private static class MessageFactory {
        ...
        private static StratioStreamingMessage getMessageFromCommand(String command, String sessionId) {
            ...
            if (!(operation.equalsIgnoreCase(STREAM_OPERATIONS.DEFINITION.ALTER)
                    ...
                    || operation.equalsIgnoreCase(STREAM_OPERATIONS.ACTION.INDEX)
                    || operation.equalsIgnoreCase(STREAM_OPERATIONS.ACTION.STOP_LISTEN) || operation
                        .equalsIgnoreCase(STREAM_OPERATIONS.MANIPULATION.LIST))) {
            ...
            if ((operation.equalsIgnoreCase(STREAM_OPERATIONS.DEFINITION.ALTER)
                    || operation.equalsIgnoreCase(STREAM_OPERATIONS.DEFINITION.CREATE)
                    || operation.equalsIgnoreCase(STREAM_OPERATIONS.MANIPULATION.INSERT)
                    || operation.equalsIgnoreCase(STREAM_OPERATIONS.DEFINITION.ADD_QUERY) || operation
                        .equalsIgnoreCase(STREAM_OPERATIONS.DEFINITION.REMOVE_QUERY)) && command.split("@").length != 3) {
            ...
            if ((operation.equalsIgnoreCase(STREAM_OPERATIONS.DEFINITION.DROP)
                    || operation.equalsIgnoreCase(STREAM_OPERATIONS.ACTION.SAVETO_CASSANDRA)
                    || operation.equalsIgnoreCase(STREAM_OPERATIONS.ACTION.LISTEN) || operation
                        .equalsIgnoreCase(STREAM_OPERATIONS.ACTION.STOP_LISTEN)) && command.split("@").length != 2) {

                throw new IllegalArgumentException("Malformed request, missing or exceding parts: " + command);
            }

            if (operation.equalsIgnoreCase(STREAM_OPERATIONS.MANIPULATION.LIST) && command.split("@").length != 1) {
                throw new IllegalArgumentException("Malformed request, missing or exceding parts: " + command);
            }
