...
//		stop all listeners
		siddhiManager.getSiddhiContext().getHazelcastInstance().getTopic(STREAMING.INTERNAL_LISTEN_TOPIC).publish(request.getStreamName());		
		
//		remove all queries
		if (StreamSharedStatus.getStreamStatus(request.getStreamName(), siddhiManager) != null) {
			HashMap<String, String> attachedQueries = StreamSharedStatus.getStreamStatus(request.getStreamName(), siddhiManager).getAddedQueries();
...
//		remove query in stream status
		String queryId = StreamSharedStatus.removeQueryInStreamStatus(request.getRequest(), request.getStreamName(), siddhiManager);
...
//				stop all listeners
				siddhiManager.getSiddhiContext().getHazelcastInstance().getTopic(STREAMING.INTERNAL_LISTEN_TOPIC).publish(streamName);
...
//				drop the streamStatus
		...
		for(StreamDefinition streamMetaData : siddhiManager.getStreamDefinitions()) {
			...
			if (!Arrays.asList(STREAMING.STATS_NAMES.STATS_STREAMS).contains(streamMetaData.getStreamId())) {
				...
				if (StreamSharedStatus.getStreamStatus(streamMetaData.getStreamId(), siddhiManager) != null) {
					HashMap<String, String> attachedQueries = StreamSharedStatus.getStreamStatus(streamMetaData.getStreamId(), siddhiManager).getAddedQueries();
					
					for (Entry<String, String> entry : attachedQueries.entrySet()) {
						queries.add(new StreamQuery(entry.getKey(), entry.getValue()));				    
					}
					
					isUserDefined = StreamSharedStatus.getStreamStatus(streamMetaData.getStreamId(), siddhiManager).isUserDefined();
				}
	...
	public static void listenStream(StratioStreamingMessage request, String kafkaCluster, SiddhiManager siddhiManager) {
		...
		StreamToBusCallback streamCallBack = new StreamToBusCallback(siddhiManager.getStreamDefinition(request.getStreamName()), kafkaCluster);
		
		ITopic<String> listenTopic =  siddhiManager.getSiddhiContext().getHazelcastInstance().getTopic(STREAMING.INTERNAL_LISTEN_TOPIC);
	...
	public static void save2cassandraStream(StratioStreamingMessage request, String cassandraCluster, SiddhiManager siddhiManager) {
		
		StreamToCassandraCallback cassandraCallBack = new StreamToCassandraCallback(siddhiManager.getStreamDefinition(request.getStreamName()), cassandraCluster);
		
		ITopic<String> save2cassandraTopic =  siddhiManager.getSiddhiContext().getHazelcastInstance().getTopic(STREAMING.INTERNAL_SAVE2CASSANDRA_TOPIC);
