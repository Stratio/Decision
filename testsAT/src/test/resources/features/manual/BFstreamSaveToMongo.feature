@ignore @manual @api
Feature: Create a stream, operate over it and persist to MongoDB

  Scenario: Data passthrough and the result is persisted to MongoDB
    Given I start a new StreamingAPI session
    When I start saving to MongoDB a stream with name '${STREAM_NAME}'
  ...
  Scenario: Data passthrough to MongoDB but MongoDB is down
    Given I start a new StreamingAPI session
    When I start saving to MongoDB a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
  ...
  Scenario: Data passthrough to MongoDB not matching model given
    Given I start a new StreamingAPI session
    When I start saving to MongoDB a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
  ...
  Scenario Outline: Data is filtered though a valid query and the result is saved to MongoDB
    Given I start a new StreamingAPI session
    When I start saving to MongoDB a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
