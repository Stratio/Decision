@ignore @manual @api
Feature: Create a stream, operate over it and persist to Cassandra

  Scenario: Data passthrough and the result is persisted to Cassandra
    Given I start a new StreamingAPI session
    When I start saving to Cassandra a stream with name '${STREAM_NAME}'
  ...
  Scenario: Data passthrough to Cassandra but Cassandra is down
    Given I start a new StreamingAPI session
    When I start saving to Cassandra a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
  ...
  Scenario: Data passthrough to Cassandra not matching model given
    Given I start a new StreamingAPI session
    When I start saving to Cassandra a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
  ...
  Scenario Outline: Data is filtered though a valid query and the result is saved to Cassandra
    Given I start a new StreamingAPI session
    When I start saving to Cassandra a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
