@ignore @manual @api
Feature: Create a stream, operate over it and persist to Solr

  Scenario: Data passthrough and the result is persisted to Solr
    Given I start a new StreamingAPI session
    When I start saving to Solr a stream with name '${STREAM_NAME}'
    ...
    Then a Solr dataBase '.+' contains a table '.+' with values:
      | name | a |
      | data1 | 1 |
      | data2 | 2.3 |
  ...
  Scenario: Data passthrough to Solr but Solr is down
    Given I start a new StreamingAPI session
    When I start saving to Solr a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
  ...
  Scenario: Data passthrough to Solr not matching model given
    Given I start a new StreamingAPI session
    When I start saving to Solr a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
  ...
  Scenario Outline: Data is filtered though a valid query and the result is saved to Solr
    Given I start a new StreamingAPI session
    When I start saving to Solr a stream with name '${STREAM_NAME}'
    And I create a stream with name '${STREAM_NAME}' and columns (with type):
      | name  | String  |
