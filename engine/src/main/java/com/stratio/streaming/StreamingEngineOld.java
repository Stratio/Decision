...
@Deprecated
public class StreamingEngineOld {

    private static Logger logger = LoggerFactory.getLogger(StreamingEngineOld.class);
    private static SiddhiManager siddhiManager;
    private static JavaStreamingContext streamingBaseContext;
    private static JavaStreamingContext streamingDataContext;
    ...
    public static void main(String[] args) throws MalformedURLException {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            ...
            public void run() {
                ...
                if (streamingDataContext != null) {
                    streamingDataContext.stop();
                }
    ...
    private static void launchStratioStreamingEngine() throws Exception {
        ...
        streamingBaseContext = new JavaStreamingContext(cc.getSparkHost(), StreamingEngineOld.class.getName(),
                new Duration(cc.getStreamingBatchTime()));
        streamingBaseContext.sparkContext().getConf()
                .setJars(JavaStreamingContext.jarOfClass(StreamingEngineOld.class));

        streamingDataContext = new JavaStreamingContext(cc.getSparkHost(), StreamingEngineOld.class.getName(),
                new Duration(cc.getStreamingBatchTime()));
        streamingDataContext.sparkContext().getConf()
                .setJars(JavaStreamingContext.jarOfClass(StreamingEngineOld.class));
        ...
        JavaPairDStream<String, String> messages = KafkaUtils.createStream(streamingBaseContext,
                cc.getZookeeperHostsQuorum(), BUS.STREAMING_GROUP_ID, baseTopicMap);

        Map<String, Integer> dataTopicMap = new HashMap<String, Integer>();
        dataTopicMap.put(BUS.TOPIC_DATA, 1);
        // Start the Kafka stream
        JavaPairDStream<String, String> dataMessages = KafkaUtils.createStream(streamingDataContext,
                cc.getZookeeperHostsQuorum(), BUS.STREAMING_GROUP_ID, dataTopicMap);
        ...
        JavaDStream<StratioStreamingMessage> insertRequests = dataMessages.filter(
